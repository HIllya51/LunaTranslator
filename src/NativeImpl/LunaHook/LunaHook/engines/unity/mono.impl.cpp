...
namespace
{
    ...
    MonoMethod *getmethodofklass_1(MonoClass *klass, const char *name, int argsCount)
    {
        ...
        if (auto s = getclassinfo(klass))
        {
            ...
            Msg::Log(getmethodinfo(MonoClassMethod).c_str());
        }
        return MonoClassMethod;
    }
    void *getmethodofklass(MonoClass *klass, const char *name, int argsCount)
    {
        auto _ = getmethodofklass_1(klass, name, argsCount);
        if (!_)
            return NULL;
        return SafeFptr(mono_compile_method)(_);
    }
    ...
    struct AutoThread
    {
        ...
        static MonoThread *attach_thread()
        {
            auto d = (SafeFptr(mono_get_root_domain))();
            if (!d)
                return nullptr;
            return (SafeFptr(mono_thread_attach))(d);
        }
        AutoThread()
        {
    ...
    {
        auto methodName = SafeFptr(mono_method_full_name)(method, true);
        if (!methodName)
            return;
        auto sig = SafeFptr(mono_method_signature)(method);
        if (!sig)
            return;
        auto cnt = SafeFptr(mono_signature_get_param_count)(sig);
        if (!cnt)
            return;
        gpointer itertype = nullptr;
        while (auto type = SafeFptr(mono_signature_get_params)(sig, &itertype))
        {
            auto tp = SafeFptr(mono_type_get_name)(type);
            if (!tp)
                continue;
            if (strcmp(tp, "System.String"))
                continue;
            auto ptr = (uintptr_t)(SafeFptr(mono_compile_method))(method);
            if (!ptr)
                continue;
...
template <typename F>
std::invoke_result_t<F, MonoClass *> get_pointer_in_class(const char *assemblyName, const char *namespaze,
...
struct mono_impl : monoil2cpp
{
    void *m_applyFont = nullptr;

    mono_impl(HMODULE game_module)
    {
    ...
    {
        return get_pointer_in_class(assemblyName, namespaze, klassName, strict, [&](MonoClass *klass)
    ...
    {
        return get_pointer_in_class(assemblyName, namespaze, klassName, strict, gettypeofklass);
    }
    ...
    std::variant<monoloopinfo, il2cpploopinfo> loop_all_methods(std::function<void(const std::string &)> show) override
    {
        ...
        for (auto klass : klasses)
        {
            ...
            {
                if (show)
                    Msg::Log(getmethodinfo(method).c_str());
                else
                {
    ...
    bool load_managed_plugin_impl() override
    {
        ...
        if (!method)
            return false;
        m_applyFont = (void *)(SafeFptr(mono_class_get_method_from_name))(klass, "ApplyFont", 1);
    ...
    void apply_font(void *self) override
    {
        ...
        void *args[1] = {self};
        void *exc = nullptr;
        invoke(m_applyFont, nullptr, args, &exc);
        if (exc)
            log_managed_exception(exc);
    }
};
