...
def downloadmapie():
    ...
    subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y magpie.zip")
    os.chdir(rootDir)
...
def downloadlr():
    ...
    subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y {base}")
    os.chdir(rootDir)
...
def downloadLocaleEmulator():
    ...
    subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y {localeEmulatorFile.split('/')[-1]} -oLocaleEmulator")
...
def downloadNtlea():
    ...
    subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y {ntleaFile.split('/')[-1]} -ontlea")
...
def downloadCurl(target):
    if target == "winxp":
        ...
        subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y {curlFile32xp.split('/')[-1]}")
        os.chdir(rootDir)
    ...
    subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y {curlFile32.split('/')[-1]}")
    subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y {curlFile64.split('/')[-1]}")
    os.chdir(rootDir)
...
def downloadOCRModel():
    ...
    subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y jazhchten.zip")
...
def downloadbass():
    ...
    for link in (
        ...
        subprocess.run(f"C:\\Program Files\\7-Zip\\7z.exe x -y {name} -o{d}")
        os.chdir(rootDir)
