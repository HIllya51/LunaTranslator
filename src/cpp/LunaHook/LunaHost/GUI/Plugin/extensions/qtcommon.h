...
{
    Settings(QObject *parent = nullptr) : QSettings(CONFIG_FILE, QSettings::IniFormat, parent) {}
};
...
inline void sanitize(QString &s)
{
    s.chop(std::distance(std::remove_if(s.begin(), s.end(), [](QChar ch)
                                        { return ch.isSurrogate(); }),
                         s.end()));
}
inline QString sanitize(QString &&s)
{
    sanitize(s);
    return std::move(s);
}
