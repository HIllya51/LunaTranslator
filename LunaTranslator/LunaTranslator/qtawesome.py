...
class CharIconEngine(QIconEngine):
    ...
    def paint(self, painter, rect ):
        self.painter.paint(
            self.iconic, painter, rect , self.char,self.color)
...
class IconicFont(QObject): 
    ...
    def icon(self,name,color):
        ...
        if cache_key not in self.icon_cache:
            ...
            self.icon_cache[cache_key] = self._icon_by_painter(self.painter, char,color)
...
def _instance(): 
    ...
    if _resource['iconic'] is None:
        _resource['iconic'] = IconicFont(  'fontawesome4.7-webfont.ttf',
