        ...
        if   self.config["path"]!=self.path or self.config["voice"]!=self.voice or globalconfig["ttscommon"]["rate"]!=self.rate:
            ...
            def linear_map(x): 
                if x >= 0 :
                    ...
                    x= 0.05 * x+ 1.0
                return x
            self.engine=autoproc(subproc_w('"{}" voiceroid2 "{}" "{}" {} 44100 {} "{}"  {} {}'.format(exepath,self.config["path"],dllpath,self.config["voice"],linear_map(globalconfig["ttscommon"]["rate"]),savepath,pipename,waitsignal),name='voicevoid2'))
