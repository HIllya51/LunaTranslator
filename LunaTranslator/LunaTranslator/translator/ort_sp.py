...
class TS(basetrans):  
    def inittranslator(self):
        ...
        model_path_candidates = glob.glob(os.path.join(path, 'translator_model', '*.onnx'), recursive=True)
        if len(model_path_candidates) > 0:
            model_path = model_path_candidates[0]
        else:
            raise Exception("onnx file not found!")

        tok_path_candidates = glob.glob(os.path.join(path,'tokenizer_model','*.model'))
        if len(model_path_candidates) > 0:
    ...
    def create_ort_tensors(self):
        ...
        res = self.ortmtlib.create_tensor_int32(self.max_length_ctypes, self.len_one, self.shape_one, self.len_one, ctypes.byref(self.max_length_tensor))

        self.min_length_ctypes = (ctypes.c_int32 * 1)(min_length)
        res = self.ortmtlib.create_tensor_int32(self.min_length_ctypes, self.len_one, self.shape_one, self.len_one, ctypes.byref(self.min_length_tensor))

        self.num_beams_ctypes = (ctypes.c_int32 * 1)(num_beams)
        res = self.ortmtlib.create_tensor_int32(self.num_beams_ctypes, self.len_one, self.shape_one, self.len_one, ctypes.byref(self.num_beams_tensor))

        self.num_return_sequences_ctypes = (ctypes.c_int32 * 1)(num_return_sequences)
        res = self.ortmtlib.create_tensor_int32(self.num_return_sequences_ctypes, self.len_one, self.shape_one, self.len_one, ctypes.byref(self.num_return_sequences_tensor))

        self.length_penalty_ctypes = (ctypes.c_float * 1)(length_penalty)
        res = self.ortmtlib.create_tensor_float(self.length_penalty_ctypes, self.len_one, self.shape_one, self.len_one, ctypes.byref(self.length_penalty_tensor))

        self.repetition_penalty_ctypes = (ctypes.c_float * 1)(repetition_penalty)
        res = self.ortmtlib.create_tensor_float(self.repetition_penalty_ctypes, self.len_one, self.shape_one, self.len_one, ctypes.byref(self.repetition_penalty_tensor))
        return

    def encode_as_ids(self, content):
        input_str = ctypes.c_char_p(bytes(f'<-{self.srclang}2{self.tgtlang}-> ' + content, "utf8"))
    ...
    def run_session(self, input_ids_py):
        ...
        res = self.ortmtlib.create_tensor_int32(input_ids_ctypes, input_ids_len_ctypes,input_shape_ctypes, input_shape_len_ctypes, ctypes.byref(input_ids_tensor))

        #self.ortmtlib.print_tensor_int32(input_ids_tensor)
        input_tensors = [input_ids_tensor, self.max_length_tensor, self.min_length_tensor, self.num_beams_tensor, self.num_return_sequences_tensor, self.length_penalty_tensor, self.repetition_penalty_tensor]
