...
async def waittransok( websocket):  
        for i in range(10000):
            state =(await SendRequest(websocket,'Runtime.evaluate',{"expression":'document.querySelector("#translation > span").innerText',"returnByValue":True}))  
...
async def tranlate(websocketurl,content,src,tgt ): 
    async with websockets.connect(websocketurl) as websocket: 
        await SendRequest(websocket,'Runtime.evaluate',{"expression":'document.querySelector("#translation > span").innerText=""'})
        await SendRequest(websocket,'Runtime.evaluate',{"expression":f'i=document.querySelector("#fakeArea");i.innerText=`{content}`;event = new Event("input", {{bubbles: true, cancelable: true }});i.dispatchEvent(event);'}) 
