...
def tranlate(websocketurl,content,src,tgt ): 
    if 1:
        ...
        SendRequest(websocket,'Runtime.evaluate',{"expression":'(a=document.querySelector("#textTrans > div.translation-box > div > div:nth-child(1) > div > i"))?a.click():"";i=document.querySelector("#textarea");i.value=`{}`;event = new Event("input", {{bubbles: true, cancelable: true }});i.dispatchEvent(event);'.format(content)}) 
