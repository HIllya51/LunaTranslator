...
class TS(basetrans):
    ...
    def get_baiducookie_token(self,max_try_nums=3): 
            headers = {
            ...
            session.get("http://www.baidu.com",headers=headers,proxies=self.proxy)
            res = session.get("https://fanyi.baidu.com/?aldtype=85#zh/en/%E4%BB%8A%E5%A4%A9%E6%98%AF%E4%B8%AA%E5%BC%80%E5%BF%83%E7%9A%84%E6%97%A5%E5%AD%90",headers=headers,proxies=self.proxy)
            ...
            if not bool(BAIDUID) or not bool(token): # 有一个没取到都会有问题
                return self.get_baiducookie_token(max_try_nums=max_try_nums-1)
            return BAIDUID[0] ,token[0]
    def baidufanyi_sign(self,src):
    ...
    def inittranslator(self):
        ...
        self.session.mount('https://fanyi.baidu.com', DESAdapter())
        self.session.proxy=self.proxy
        self.BAIDUID,self.token = self.get_baiducookie_token()
    def translate(self,query):  
        self.session.proxy=self.proxy
        
        sign = self.baidufanyi_sign(query)
        
        # print(sign,BAIDUID,token)
        url = "https://fanyi.baidu.com/v2transapi"

        payload = f"from={self.srclang}&to={self.tgtlang}&query={quote(query)}&transtype=realtime&simple_means_flag=3&sign={sign}&token={self.token}&domain=common"
        headers = {
        ...
        return "error on translation" if len(re_result) < 1 else '\n'.join(_['dst'] for _ in re_result)#[0].get("dst","")
