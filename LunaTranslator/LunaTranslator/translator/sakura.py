...
class TS(basetrans):
    ...
    def make_request(self, prompt):
        if self.api_type == "llama.cpp":
            request = {
                "prompt": prompt,
                "n_predict": int(self.config['max_new_token']),
    ...
    def set_api_type(self):
        ...
        try:
            response = self.session.post(api_url, json=test_json).json()
            output = response['results'][0]['text']
            new_token = response['results'][0]['new_token']
