...
class TS(basetrans):
    ...
    def checkstates(self):
        try:
            self.checkfilechanged(
                self.unsafegetcurrentgameconfig(), self.config["xml文件"]
            )
        except:
            pass
    ...
    def translate(self, content):
        ...
        if globalconfig["premtsimiuse"]:
            ...
            for jc in self.json:
                ...
                if dis < mindis:
                    ...
                    if mindis < globalconfig["premtsimi"]:
                        if (
                            self.config[jc]["userTrans"]
                            and self.config[jc]["userTrans"] != ""
                        ):
                            savet = self.config[jc]["userTrans"]

                        elif (
                            self.config[jc]["machineTrans"]
                            and self.config[jc]["machineTrans"] != ""
                        ):
            ...
            if (
                self.config[content]["userTrans"]
                and self.config[content]["userTrans"] != ""
            ):
                return self.config[content]["userTrans"]

            elif (
                self.config[content]["machineTrans"]
                and self.config[content]["machineTrans"] != ""
            ):
