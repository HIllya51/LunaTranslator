...
class TS(basetransdev): 
    ...
    def translate(self,content):
        ...
        num=self.wait_for_result('''document.querySelector("#html2canvas").children.length''')
        if num==1:
            num=0
        ...
        if self.config['流式输出']==False:
            while 1:
                ...
                if 'stop' not in self.wait_for_result(r'''document.querySelector("#INPUT > div.md\\:bg-white.md\\:dark\\:bg-n-6 > div.flex.justify-center.relative > button").innerText''').lower():
                    break
            yield self.wait_for_result('''document.querySelector("#html2canvas > div:nth-child({}) > div.markdown.overflow-x-auto.prose").textContent'''.format(num+2))
        else:
            currtext=''
            while True:
                time.sleep(0.1)
                newcurr=self.wait_for_result('''document.querySelector("#html2canvas > div:nth-child({}) > div.markdown.overflow-x-auto.prose").textContent'''.format(num+2))
                if newcurr=='...':continue
