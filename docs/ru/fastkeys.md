...
1. #### Отключить все остальные горячие клавиши {#anchor-disableothers}
    Отключает все остальные горячие клавиши, кроме этой. Повторное нажатие этой горячей клавиши снимает отключение.
...
1. #### Прозрачный фон окна {#anchor-51}
    Эта кнопка служит только для мгновенного переключения непрозрачности окна перевода на 0. Когда непрозрачность равна 0, мышь также будет проникать сквозь окно.
...
1. #### Скрыть панель инструментов {#anchor-52}
    Скрывает панель инструментов. Этот параметр имеет более высокий приоритет, чем блокировка панели инструментов.
...
1. #### Автокликер {#anchor-53}
    (Пропущено)

1. #### Сбросить состояние перевода {#anchor-45}
    Сбрасывает состояние перевода, в основном предназначен для растущих потребностей в переводе больших моделей, позволяет очистить сохраненный контекст и другую информацию.

1. #### Немедленно сохранить конфигурацию {#anchor-50}
    Немедленно сохранить текущую конфигурацию пользователя, а не ждать до выхода.

1. #### Отказаться от фокуса окна {#anchor-giveupfocus}
    Переводит все окна программы в состояние, в котором они не могут получать фокус, чтобы при работе с окном они не перехватывали фокус у игры, что повышает погружение.

    Кроме того, в некоторых случаях, например, некоторые старые игры при потере фокуса выходят из полноэкранного режима — эта функция может смягчить эту проблему.

    При повторном нажатии горячей клавиши исходное состояние восстанавливается, и окно снова сможет получать фокус в обычном режиме.
...
1. #### Создание нескольких OCR-областей Переключиться на область рядом с курсором {#anchor-48}
    **Доступно только в режиме OCR**
    ...
    После активации режима нескольких зон можно использовать это сочетание клавиш, чтобы переключить фокус на зону возле курсора мыши и немедленно повторно выполнить OCR.
...
1. #### Снимок окна {#anchor-_21}
    Если в данный момент используется масштабирование Magpie, скриншот будет сделан увеличенного окна.

    После привязки игрового окна можно сделать скриншот привязанного окна. Сначала используется захват GDI, а при неудаче — Windows.Capture.

    По умолчанию левый клик сохраняет снимок в файл, правый клик — в буфер обмена. Функции левого и правого клика можно поменять местами в настройках.
...
1. #### Масштабирование окна Полноэкранный режим масштабирования {#anchor-41}
...
1. #### Масштабирование окна Режим масштабирования окна {#anchor-42}
