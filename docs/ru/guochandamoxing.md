...
1. #### Пользовательский system prompt / Пользовательское user message / prefill  {#anchor-prompt}
    ...
    - `{contextOriginal[N]}` и `{contextTranslation[N]}` и `{contextBoth[N]}`: N элементов исторического текста, перевода и обоих. Если на вход подаётся `contextBoth[N]`, будет использовано значение `附带上下文个数`; если на вход подаётся `contextBoth[10]`, будет использовано введённое количество — 10.
...
1. #### thinking.type
    Переключатель режима мышления, поддерживаемый некоторыми платформами (в основном DeepSeek).
