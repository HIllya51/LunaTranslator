...
1. #### Пользовательский system prompt / Пользовательское user message / prefill  
    ...
    В пользовательских сообщениях вы можете ссылаться на записи из `Перевода собственных существительных` с помощью следующих полей. **Если совпадающая запись не найдена, эти поля будут очищены, чтобы избежать нарушения содержимого перевода**.  
    - `{DictWithPrompt[XXXXX]}`, где `XXXXX` — это промпт, который направляет LLM на использование данных записей для оптимизации перевода. Вы можете определить его самостоятельно или отключить пользовательские сообщения, чтобы использовать промпт по умолчанию.
