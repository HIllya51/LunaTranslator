...
1. #### Temperature / max tokens / top p / frequency penalty  
    ...
    - **Использовать max completion tokens** — для платформы OpenAI при использовании модели gpt-5 параметр max tokens не поддерживается, поэтому необходимо активировать **Использовать max completion tokens**.  
