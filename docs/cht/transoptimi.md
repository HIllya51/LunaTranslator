...
1. ## 專有名詞翻譯 {#anchor-noundict}
    ...
    針對`大模型通用接口`，如果其提示詞中包含`DictWithPrompt`，則會將詞條放入模型的提示詞中。使用方法請參考[此文件](/zh/guochandamoxing.html#anchor-prompt)。
        
    對於其他傳統翻譯，或`大模型通用接口`的提示詞中不包含`DictWithPrompt`，則會參考VNR的做法，將原文替換為佔位符`ZX?Z`（附註：我也不知道這是什麼意思），翻譯來源翻譯後一般不會破壞佔位符，然後在翻譯後將佔位符替換成翻譯。

    對於遊戲的專用詞條，建議不要在文字處理->翻譯最佳化中新增。建議在`遊戲設定`中的`翻譯最佳化`的該方法的設定中，新增遊戲專用的詞條。
