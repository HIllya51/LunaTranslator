...
1. ## 專有名詞翻譯 {#anchor-noundict}
    ...
    當從VNDB加載元數據時，遊戲會查詢角色名稱信息並設定為預設詞典。對於英文用戶，提取的英文名稱會被填入作為原文對應的翻譯。否則，為了在用戶未進行修改時不影響翻譯，翻譯內容將填入與原文相同的內容。
...
1. ## 專有名詞翻譯 {#anchor-noundict}
    ...
    遊戲從VNDB加載元數據時，會查詢遊戲中的人物名稱信息作為預設詞典。對於英文用戶，會將提取到的英文填入作為原文對應的翻譯，否則會將翻譯留空，以避免在用戶未進行修改時影響翻譯。
