...
1. #### 停用其他所有快速鍵 {#anchor-disableothers}
    停用除了此快速鍵之外的其他全部快捷鍵。再次觸發此快捷鍵會解除停用。
...
1. #### 放棄視窗焦點 {#anchor-giveupfocus}
    使程式的所有視窗都切換到無法獲得焦點的狀態，從而在操作視窗時不會與遊戲爭奪焦點，增加沉浸感。

    並且某些情況下，例如一些老舊遊戲在全螢幕時如果失去焦點會退出全螢幕，此功能或可緩解此問題。

    再次觸發快速鍵時恢復原始狀態，視窗將可以正常獲得焦點。
...
1. #### 多重區域模式 切換到滑鼠游標鄰近區域 {#anchor-48}
    **僅在OCR模式下可用**

    啟動多重區域模式後，可以使用該快速鍵，將聚焦的區域切換到鼠標鄰近區域，並立即重新進行 OCR。
...
1. #### 視窗縮放 全螢幕模式縮放 {#anchor-41}
    當綁定遊戲視窗後，可以一鍵對遊戲視窗使用內建的 Magpie 進行全螢幕縮放。

1. #### 視窗縮放 視窗模式縮放 {#anchor-42}
