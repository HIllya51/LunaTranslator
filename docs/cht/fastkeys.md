...
1. #### 鎖定工具列 {#anchor-_9}
    ...
    如果目前使用了視窗特效（Aero/Arylic），且不鎖定工具列，則工具列會處於文字區的 z 軸之上的區域，而非處於文字區的 y 軸上面。這是因為由於 Windows 的原因，使用視窗特效時，如果將工具列只是隱藏而非將縮去其視窗高度，則被隱藏的工具列仍會被渲染亞力克/Aero 背景，導致工具列所在區域會有一塊空白。

1. #### 取詞翻譯 {#anchor-38}
    對目前滑鼠游標選取到的文字進行翻譯

    優先使用 UIAutomation 進行取詞。如果目前視窗的焦點控件不支援 UIAutomationTextPattern 導致取詞失敗，則讀取剪貼簿。
...
1. #### 進行一次 OCR {#anchor-_26}
    ...
    一般用於，在 HOOK 模式下，遇到選擇支時，臨時使用一次 OCR 進行翻譯選擇支。或者在 OCR 模式下，臨時去識別一次其他偶爾出現的新的位置。


1. #### 再次進行 OCR {#anchor-_26_1}
    當使用過一次`進行一次 OCR`後，使用這個快速鍵，可以在原來的位置上再次進行一次 OCR 而無需重新選擇識別區域。
...
1. #### 朗讀 {#anchor-_7}
    ...
    該朗讀會無視`跳過`（如果在`語音指定`中，比對目前文字目標為`跳過`，則使用快速鍵進行朗讀時，會無視跳過，強制進行朗讀）
...
1. #### 綁定視窗 （點擊自己取消） {#anchor-_15}
    ...
    在 OCR 模式下，綁定視窗後，還額外允許遊戲視窗移動時，同步自動移動 OCR 區域和範圍框。
    在 OCR／剪貼簿模型下，綁定視窗後，也可以和 HOOK 模式下一樣，關聯到目前遊戲到遊戲設定，從而使用目前遊戲的專有翻譯優化詞典等。
...
1. #### Magpie 縮放 {#anchor-41}
    可以一鍵對遊戲視窗使用內建的 Magpie 進行全螢幕縮放

1. #### Magpie 視窗化縮放 {#anchor-42}
    可以一鍵對遊戲視窗使用內建的 Magpie 進行視窗化縮放
...
1. #### 取詞查詞 在新視窗中 {#anchor-40}
    對目前滑鼠游標選取到的文字在新建的查詞視窗中查詞，以避免覆蓋正在進行的查詞
...
1. #### Anki 錄音 例句 {#anchor-_30}
    查詞視窗中的 Anki 新增介面中的錄音功能的快速鍵，但這個快速鍵錄製的音訊將設定給例句欄位
