...
註：為了實現 HOOK 擷取遊戲文字，需要將 DLL 注入到遊戲，`LunaSubprocess32.exe`、`LunaHost32.dll`等幾個檔案中實現了這些內容，因此特別容易被認為是病毒。軟體目前由 [Github Actions](https://github.com/HIllya51/LunaTranslator/actions) 自動建構，除非 Github 伺服器中毒了，否則不可能包含病毒，因此可以放心地新增例外。
