...
2. **當遊戲視窗移動時，OCR 區域會跟隨遊戲視窗同步移動**。這樣，當有時需要移動遊戲視窗時，不需要移動 OCR 範圍框了，尤其是當您隱藏了範圍框時，不需要顯示->移動->再隱藏了。
...
3. 可以使用內建的 Magpie，或使用工具按鈕呼叫您自己下載的 Magpie

4. 可以從視窗控制代碼取得遊戲位置以及軟體內部 ID，可以對遊戲進行一些個人化的設定，包括專用於該遊戲的語言／語言合成／翻譯優化／文字處理／Anki 等的設定等
