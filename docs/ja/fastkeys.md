...
1. #### 他のすべてのショートカットキーを無効にする {#anchor-disableothers}
    このショートカット以外のすべてのショートカットを無効にします。再度このショートカットを実行すると、無効状態が解除されます。
...
1. #### ツールバーを非表示にする {#anchor-52}
    ツールバーを非表示にします。この設定は、ツールバーのロックよりも優先されます。
...
1. #### ウィンドウフォーカスを放棄する {#anchor-giveupfocus}
    プログラムのすべてのウィンドウをフォーカスを取得できない状態に切り替えます。これにより、ウィンドウを操作する際にゲームとフォーカスを奪い合うことがなくなり、没入感が高まります。

    また、一部の古いゲームでは、フルスクリーン時にフォーカスを失うとフルスクリーンが解除されることがありますが、この機能でその問題を緩和できる場合があります。

    もう一度ショートカットキーを実行すると元の状態に戻り、ウィンドウは通常どおりフォーカスを取得できるようになります。
...
1. #### ウィンドウスクリーンショット {#anchor-_21}
    ...
    デフォルトでは、左クリックでスクリーンショットをファイルに保存し、右クリックでクリップボードに保存します。右クリックと左クリックの機能は設定で切り替えることができます。
...
1. #### ウィンドウ拡大縮小 全画面モードでズーム {#anchor-41}
    ゲームウィンドウをバインド後、ゲームウィンドウに内蔵のMagpieを使用してワンクリックでフルスクリーンスケーリングが可能です。

1. #### ウィンドウ拡大縮小 ウィンドウモードのズーム {#anchor-42}
