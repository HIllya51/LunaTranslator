...
1. #### APIキー  
    ...
    **APIキー**は各プラットフォームで取得できます。複数のキーを追加した場合、自動的にローテーションされ、エラーフィードバックに基づいてキーの重みが調整されます。  

1. #### モデル  

    ほとんどのプラットフォームでは、**APIインターフェースアドレス**と**APIキー**を入力後、**モデル**横の更新ボタンをクリックすると、利用可能なモデルリストを取得できます。  
...
1. #### Temperature / max tokens / top p / frequency penalty  
    ...
    - **max completion tokensを使用** - OpenAIプラットフォームでGPT-5モデルを使用する場合、インターフェースがmax tokensパラメータを受け付けないため、**max completion tokensを使用**を有効にする必要があります。  

1. #### reasoning effort  

    Geminiプラットフォームでは、このオプションをGeminiのthinkingBudgetに自動的にマッピングします。マッピングルールは次の通りです：
