...
1. #### カスタムsystem prompt / カスタムuser message / プリフィル  
    ...
    カスタムユーザーメッセージでは、以下のフィールドを使用して`固有名詞翻訳`のエントリを参照できます。**一致するエントリがない場合、これらのフィールドはクリアされ、翻訳内容が破壊されるのを防ぎます**。  
    - `{DictWithPrompt[XXXXX]}`。ここで、`XXXXX`はLLMに提供されたエントリを使用して翻訳を最適化するよう導くプロンプトです。独自に定義するか、カスタムユーザーメッセージを無効にしてデフォルトのガイドプロンプトを使用できます。
