...
1. #### 사용자 정의 system prompt / 사용자 정의 user message / prefill  
    ...
    사용자 정의 사용자 메시지에서 다음 필드를 사용하여 `고유 명사 번역`의 항목을 참조할 수 있습니다. **일치하는 항목이 없으면 이러한 필드는 번역 내용을 손상시키지 않도록 지워집니다**.  
    - `{DictWithPrompt[XXXXX]}`, 여기서 `XXXXX`는 LLM에 제공된 항목을 사용하여 번역을 최적화하도록 유도하는 프롬프트입니다. 직접 정의하거나 사용자 정의 사용자 메시지를 비활성화하여 기본 가이드 프롬프트를 사용할 수 있습니다.
