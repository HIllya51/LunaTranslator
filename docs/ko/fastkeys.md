...
1. #### 다른 모든 단축키를 비활성화합니다. {#anchor-disableothers}
    이 단축키를 제외한 다른 모든 단축키를 비활성화합니다. 이 단축키를 다시 실행하면 비활성화가 해제됩니다.
...
1. #### 도구 모음 숨기기 {#anchor-52}
    도구 모음을 숨깁니다. 이 설정은 도구 모음 잠금보다 우선순위가 높습니다.
...
1. #### 창 포커스 해제 {#anchor-giveupfocus}
    프로그램의 모든 창을 포커스를 받을 수 없는 상태로 전환하여, 창을 조작할 때 게임과 포커스를 다투지 않게 하여 몰입감을 높입니다.

    또한 일부 오래된 게임은 전체 화면에서 포커스를 잃으면 전체 화면에서 빠져나오는 경우가 있는데, 이 기능으로 이 문제를 완화할 수 있습니다.

    단축키를 다시 실행하면 원래 상태로 복구되어 창이 정상적으로 포커스를 받을 수 있게 됩니다.
...
1. #### 창 확대/축소 전체 화면 모드 확대/축소 {#anchor-41}
    게임 창을 바인딩한 후, 내장된 Magpie를 사용하여 게임 창을 전체 화면으로 스케일링할 수 있습니다.

1. #### 창 확대/축소 창 모드 확대/축소 {#anchor-42}
