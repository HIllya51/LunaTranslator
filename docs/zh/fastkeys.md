...
1. #### 禁用其他全部快捷键 {#anchor-disableothers}
    禁用除了此快捷键之外的其他全部快捷键。再次触发此快捷键会解除禁用。
...
1. #### 放弃窗口焦点 {#anchor-giveupfocus}
    使得程序的所有窗口都切换到无法获得焦点的状态，从而在不会在操作窗口时和游戏争夺焦点，增加沉浸感。

    并且某些情况下，例如一些古早游戏在全屏时如果失去焦点会退出全屏，此功能或可缓解此问题。

    再次触发快捷键时恢复原始状态，窗口将可以正常获得焦点。
...
1. #### 绑定窗口 （点击自己取消） {#anchor-_15}
    ...
    在HOOK模式下，会自动根据连接的游戏，自动绑定游戏窗口。但也可以用该快捷键重新选择其他窗口。
...
1. #### 窗口缩放 全屏模式缩放 {#anchor-41}
    当绑定游戏窗口后，可以一键对游戏窗口使用内置的Magpie进行全屏缩放

1. #### 窗口缩放 窗口模式缩放 {#anchor-42}
