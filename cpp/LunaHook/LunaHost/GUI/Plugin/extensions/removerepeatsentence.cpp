...
BOOL WINAPI DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		if (wchar_t *fileName = wcsrchr(filePath, L'\\'))
			swscanf_s(fileName, L"\\Remove %d Repeated Sentences.xdll", &sentenceCacheSize);
	}
	break;
...
bool ProcessSentence(std::wstring &sentence, SentenceInfo sentenceInfo)
{
	...
	static std::mutex m;
	m.lock();
	if (textNumber + 1 > cache.size())
		cache.resize(textNumber + 1);
	...
	if (prevSentences->size() > sentenceCacheSize)
		prevSentences->erase(prevSentences->begin());
