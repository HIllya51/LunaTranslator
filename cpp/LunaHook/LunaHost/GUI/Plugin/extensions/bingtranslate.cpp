...
std::pair<bool, std::wstring> Translate(const std::wstring &text, TranslationParam tlp)
{
	if (!tlp.authKey.empty())
	{
		...
		if (HttpRequest httpRequest{
			...
			if (auto translation = Copy(JSON::Parse(httpRequest.response)[0][L"translations"][0][L"text"].String()))
	...
	if (HttpRequest httpRequest{
		...
		if (auto translation = Copy(JSON::Parse(httpRequest.response)[0][L"translations"][0][L"text"].String()))
			...
			return {false, FormatString(L"%s (token=%s): %s", TRANSLATION_ERROR, std::exchange(token.Acquire().contents, L""), httpRequest.response)};
	else
		return {false, FormatString(L"%s (code=%u)", TRANSLATION_ERROR, httpRequest.errorCode)};
}
