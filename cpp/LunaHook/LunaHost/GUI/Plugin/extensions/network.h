		...
		for (int i = 0; i < text.size(); ++i)
			if (text[i] == '&')
				for (auto [original, length, replacement] : Array<const C *, size_t, C>{
						 ...
						 {quot, std::size(quot), '"'},
						 {amp, std::size(amp), '&'}})
					if (std::char_traits<C>::compare(text.data() + i, original, length) == 0)
						text.replace(i, length, 1, replacement);
		return text;
	}
}
...
namespace JSON
{
	...
	std::basic_string<C> Escape(std::basic_string<C> text)
	{
		...
		text.resize(text.size() + std::count_if(text.begin(), text.end(), [](C ch)
												{ return ch == '\n' || ch == '\r' || ch == '\t' || ch == '\\' || ch == '"'; }));
		...
		text.erase(std::remove_if(text.begin(), text.end(), [](uint64_t ch)
	...
	Value<C> Parse(const std::basic_string<C> &text, int64_t &i, int depth)
	{
		...
		if (ch == nullStr[0])
			if (std::char_traits<C>::compare(text.data() + i, nullStr, std::size(nullStr)) == 0)
				return i += std::size(nullStr), nullptr;
			else
				return {};
		if (ch == trueStr[0])
			if (std::char_traits<C>::compare(text.data() + i, trueStr, std::size(trueStr)) == 0)
				return i += std::size(trueStr), true;
			else
				return {};
		if (ch == falseStr[0])
			if (std::char_traits<C>::compare(text.data() + i, falseStr, std::size(falseStr)) == 0)
				return i += std::size(falseStr), false;
			else
				return {};
		...
		if (ch == '[')
		{
			...
			while (true)
			{
				...
				if (SkipWhitespace())
					return {};
				if (ch == ']')
					return i += 1, Value<C>(array);
				if (!array.emplace_back(Parse<C, maxDepth>(text, i, depth + 1)))
					return {};
				if (SkipWhitespace())
					return {};
				if (ch == ']')
		...
		if (ch == '{')
		{
			...
			while (true)
			{
				...
				if (SkipWhitespace() || ch != ':')
					return {};
				i += 1;
				if (!(object[std::move(key)] = Parse<C, maxDepth>(text, i, depth + 1)))
					return {};
				if (SkipWhitespace())
					return {};
				if (ch == '}')
