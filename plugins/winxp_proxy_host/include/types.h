...
#else
	void(*text_fun)(uintptr_t stack, HookParam* hp, BYTE obsoleteAlwaysZero, uintptr_t* data, uintptr_t* split, uintptr_t* len);
#endif
...
struct HookFoundNotif_2 // From dll
{
	HookFoundNotif_2(HookParam hp, wchar_t* text) : hp(hp) { wcsncpy_s(this->text, text, MESSAGE_SIZE - 1); }
	HostNotificationType command = HOST_NOTIFICATION_FOUND_HOOK_2;
	HookParam hp;
	wchar_t hookcode[500];
	wchar_t text[MESSAGE_SIZE] = {}; // though type is wchar_t, may not be encoded in UTF-16 (it's just convenient to use wcs* functions)
};
